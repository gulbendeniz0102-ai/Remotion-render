...
// ── PERIOD ANCHOR ───────────────────────────────────────────────────────────
// Flux renders the present day unless told otherwise, and nothing in the image
// path had ever told it. `the-aeneid` beat 66 — "a warrior carrying a frail old
// man on his back and leading a small boy by the hand through burning streets"
// — came back photoreal and completely modern: jeans, work boots, a cardigan, a
// plastic bottle and a pram in frame, and the thumbnail's "Bronze Age warrior"
// arrived in medieval plate. The story bible has known the book's era since
// plan-bible, so read it here and put it in every prompt. `world.imageStyle`
// overrides the generated clause when a book needs a bespoke wardrobe note.
// Books with no bible (or no era) are unaffected — the clause is empty.
const STORY_BIBLE = (() => {
  try { return JSON.parse(fs.readFileSync(`books/${SLUG}/story-bible.json`, "utf8")); } catch { return null; }
})();
const PERIOD = (() => {
  const w = (STORY_BIBLE && STORY_BIBLE.world) || {};
  if (w.imageStyle) return String(w.imageStyle);
  if (!w.era) return "";
  return `set in ${w.era}, historically accurate costume, materials and props for that period, no modern clothing, no modern objects, no anachronisms`;
})();

// Words the Flux endpoint's safety filter refuses for no discernible reason.
// A filtered prompt is not a soft failure: three attempts later the beat's image
// is dropped and the scene renders text-only. Verified by bisecting one prompt
// word by word — "papyrus" is refused on its own ("wool and linen", "sandals",
// "oil lamps", "Bronze-Age Troy" all pass), which is what silently cost
// the-aeneid three of its manuscript scenes. Add a word here when you catch one.
const FILTER_SWAPS = [[/\bpapyrus\b/gi, "rolled"]];
const deFilter = (s) => FILTER_SWAPS.reduce((acc, [re, to]) => acc.replace(re, to), s);
...
function imagePrompt(subject, style) {
  ...
  // The period is stated in full up front and re-stated in three words at the
  // end: one early mention in a long prompt gets diluted (that is how "Bronze
  // Age" still produced plate armour), but repeating the whole clause blows the
  // endpoint's 800-character prompt limit (HTTP 422 string_too_long).
  const era = PERIOD ? `${PERIOD}, ` : "";
  const eraTail = PERIOD ? ", period-accurate, no anachronisms" : "";
  const prompt = deFilter(`cinematic editorial still: ${subject}. ${era}${iso}${palette}, shallow depth of field, ${NO_TEXT}, no watermark, high detail, 8k${eraTail}`);
  // Hard cap: the Flux endpoint rejects >800 chars outright, and a rejected
  // prompt costs a whole beat's image (it is dropped as "missing" three
  // attempts later). Trim the style tail, never the subject.
  return prompt.length <= 800 ? prompt : `${prompt.slice(0, 800 - eraTail.length - 1).replace(/,\s*[^,]*$/, "")}${eraTail}`;
}
