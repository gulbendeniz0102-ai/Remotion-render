...
if (fs.existsSync(promptPath)) {
  ...
  // Where the authored opening line actually lives. In a Claude-authored prompt it is
  // the ANGLE's `- Open on this idea: "..."` line; "COLD OPEN" appears later, in the
  // fixed STRUCTURE spine, which carries no quoted line — so the COLD-OPEN-anchored
  // pattern alone matched nothing and this whole check silently skipped on every
  // hand-authored book (the-iliad, the-odyssey, the-aeneid …). Try the angle line first.
  const coMatch =
    pm.match(/Open on this idea:\s*"([^"]{10,})"/i) ||
    pm.match(/COLD OPEN[\s\S]*?:\s*"([^"]{10,})"/i);
  if (coMatch) {
